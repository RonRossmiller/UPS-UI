...
from outbound_yard.sensors import sensors
...
class UPSTeacher(Teacher):
    ...
    async def compute_reward(self, transformed_obs, action, sim_reward):
        ...
        if sim_reward == 0:
            ...
            reward = math.exp(-1e-6 * np.sum(self.outbound_list)) +  math.exp(-0.1 * np.sum(self.total_yard))

        # Penalty for repeating the same action
        if (action not in [4,5,7]) and (action == self.last_action):
            reward = -1
